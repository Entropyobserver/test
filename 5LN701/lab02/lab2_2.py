...
#nltk.download('punkt')
#import nltk
#nltk.data.path.append('/home/yaxi4987/nltk_data')
#Downloading package punkt to /home/yaxi4987/nltk_data...
def get_lines(path):
...
def main():
    ...
    perplexity = get_perplexity(bi_sur_dict, train)
...
#s = -log2(p)This equation is used to compute the surprisal for each bigram based on its probability.
def get_surprisal(p):
    s = -log(p,2)
    return s
"""
...
def get_bigram_surprisal(unigram_dict,bigram_dict):
    ...
    sur_dict = {}
    for bi_word in bigram_dict:
        first_word = bi_word[0]
        con_p = bigram_dict.get(bi_word,1) / unigram_dict.get(first_word,1)
        s = get_surprisal(con_p)
        sur_dict[bi_word] = s#?
    return sur_dict
...
def get_perplexity(bi_surp, train):
    ...
    for sentence in test:
        ...
        total_surp += bi_surp.get(first_bigram, 1 / total_surp)

        for i in range(1, len(words)-1):
            bigram = (words[i], words[i+1])
        #    if bigram in bi_surp:
        #        total_surp += bi_surp[bigram]
            total_surp += bi_surp.get(first_bigram, 1 / total_bigrams)
